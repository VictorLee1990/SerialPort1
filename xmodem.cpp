...
bool XMODEM::sendFile(const QString &filePath)
{
    ...
    file.size();
   qDebug() << "file.size: " << QString().sprintf("%d",file.size());
    char dataBuffer[128];
    m_packetNumber = 1;
...
//    packet.append((char)crc & 0xFFUL);
//    packet.append((char)((crc & 0xFF00UL) >> 8) );
